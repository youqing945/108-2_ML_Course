...
def ml_loop(side: str):
    ...
    ball_served = False
    def move_to(player, pred, frame) : #move platform to predicted position to catch ball 
        if player == '1P':
            if frame < 2 :
    ...
    def ml_loop_for_1P(): 
        if scene_info["ball_speed"][1] > 0 : # ball goes down
            x = ( scene_info["platform_1P"][1]-scene_info["ball"][1] ) // scene_info["ball_speed"][1] # x means how many frames before catch the ball
            pred = scene_info["ball"][0]+(scene_info["ball_speed"][0]*x) # pred means predict ball landing site 
            ...
            return move_to(player = '1P',pred = pred, frame = x)
        else : # ball goes up
            return move_to(player = '1P',pred = 100, frame = 3)



    def ml_loop_for_2P():  # as same as 1P
        if scene_info["ball_speed"][1] > 0 : 
            return move_to(player = '2P',pred = 100, frame = 10)
        else : 
            ...
            if (bound > 0):
                ...
                if bound%2 ==1:
                    ...
                    pred = pred + (abs(bound)*200)
            return move_to(player = '2P',pred = pred, frame = x)
